...
| 6 | AI not actually necessary: the decision core is classical; without the unstructured-evidence channel this is a Bayesian decision system with an agent interface | **HIGH** | Yes — implement the complaint-text→likelihood channel with measured calibration (reliability diagrams) plus LLM hypothesis expansion; if calibration fails, publish that as a finding rather than hiding it |
...
The agent matures through a hybrid simulation service (hot re-simulation, out-of-order delayed evidence, action queue with latencies), a posterior-reconstructible ledger with counterfactual replay, an evaluation harness with literature baselines, observability, and an operator interface. Higher-confidence evidence would include comparisons with Ji-2022-style and Sandia-P1/P2-style baselines, a reliability-diagram-backed LLM evidence channel, a released mixed-class costed-action benchmark, and demonstrated misspecification handling in which the system reports that no hypothesis fits rather than confidently converging.
