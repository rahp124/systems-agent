...
`water_investigation.ensemble` now creates 40 seeded scenarios per event class by default and writes compressed pressure, flow, and quality tensors plus scenario metadata to `artifacts/net3-ensemble.npz`. A no-event baseline trace is stored alongside them so action extraction measures event deviations rather than confusing chemical concentration with Net3's default water-age channel. The builder also accepts `net3` or `ltown` and `primary` or `alternate` sensor layouts; those settings are stored with each artifact and copied into evaluation reports. The sensor-fault injection derives its trace index from the returned cadence, so Net3's hourly and L-Town's five-minute traces align to the same event time.
...
`water_investigation.replication` evaluates separately generated network/layout artifacts without pooling scenarios. The initial report covers a 36-scenario Net3 alternate-layout ensemble and a 36-scenario L-Town primary-layout ensemble. It is intentionally a small feasibility replication: each policy sees 100 sampled held-out episodes per artifact, and no across-network interval or generalization claim is made.

## Prespecified independent benchmark

`benchmark_protocol.json` freezes five independent ensemble definitions, generation and evaluation seeds, ambiguity and conclusion thresholds, policy names, the risk penalty, sample sizes, and bootstrap settings before final results are generated. `water_investigation.final_benchmark` builds missing artifacts, evaluates every policy on identical episode sequences within each ensemble, and reports absolute accuracy, cost, action count, hierarchical confidence intervals, paired policy differences, confusion counts, and common failed action sequences.

The first run evaluates 2,000 paired episode draws per policy across two independently seeded Net3-primary ensembles, Net3 alternate, L-Town primary, and L-Town alternate. EIG-per-cost reaches 85.9% aggregate accuracy, but neither its accuracy nor cost advantage over random is statistically resolved under the ensemble-level bootstrap. Net3 alternate performance is substantially worse than the other layouts. The result therefore strengthens the evaluation method while weakening the policy-superiority claim—exactly the distinction the independent design is intended to reveal.
